...
async function req<T>(path: string, opts: RequestInit = {}): Promise<T | null> {
  const res = await fetch(`${API}${path}`, {
    credentials: 'include',   // REQUIRED — sends the httpOnly auth cookie cross-port
    cache: 'no-store',        // prevents Next.js from caching a stale 401/200
    ...opts,
  })

  if (res.status === 401) {
    return null   // caller decides what to do — do NOT redirect from inside here
  }
